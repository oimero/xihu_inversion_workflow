...
def create_ax_client(num_iters: int, random_ratio: float = 0.6, verbose: bool = False, **kwargs) -> AxClient:
    ...
    ax_gen_startegy = GenerationStrategy(
        [
            ...
            GenerationStep(
                ...
                num_trials=n_bayes,
                max_parallelism=3,
                model_kwargs={"torch_dtype": torch.float, "torch_device": device},
            ),
        ]
    )

    return AxClient(generation_strategy=ax_gen_startegy, verbose_logging=verbose, **kwargs)
